...
func (f *ApiService) Get (ctx context.Context,r *rest.Request) (interface{}, error) {
	logger.Finest("ApiService get function");
	...
	switch action {
	case "list":
		data, count, err = models.GetApiServiceLists(siteid, appid, stypes,contents,order, sort, num, start)
	case "count":
		count, err = models.GetApiServiceCount(siteid, appid, stypes, contents, order, sort)
	default:
		if id > 0{
			data, count, err = models.GetApiService(id)
		}
	}
...
func (f *ApiService) Post (ctx context.Context,r *rest.Request) (interface{}, error) {
	logger.Finest("ApiService Post function");
	...
	id, err := models.PostApiService(param)
	if err != nil{
		maps["ret"] = 1
		maps["error"] = err.Error()
		return maps, nil
...
func (f *ApiService) Put (ctx context.Context,r *rest.Request) (interface{}, error) {
	logger.Finest("ApiService Put function");
	...
	cnt, err := models.PutApiService(id, param)
	if err != nil{
		maps["ret"] = 1
		maps["error"] = err.Error()
		return maps, nil
...
func (f *ApiService) Delete (ctx context.Context,r *rest.Request) (interface{}, error) {
	logger.Finest("ApiService Delete function");
	...
	num, err := models.DeleteApiService(param)
	if err != nil{
		maps["ret"] = 1
		maps["error"] = err.Error()
		return maps, nil
