...
func (f *ApiPackageService) Get (ctx context.Context,r *rest.Request) (interface{}, error) {
	logger.Finest("ApiPackageService get function");
	...
	switch action {
	case "list":
		data, count, err = models.GetApiPkgServiceLists(siteid, appid, stypes,contents,order, sort, num, start)
	case "count":
		count, err = models.GetApiPkgServiceCount(siteid, appid, stypes, contents, order, sort)
	default:
		if id > 0{
			data, count, err = models.GetApiPkgService(id)
		}
	}
...
func (f *ApiPackageService) Post (ctx context.Context,r *rest.Request) (interface{}, error) {
	logger.Finest("ApiPackageService Post function")
	...
	id, err := models.PostApiPkgService(param)
	if err != nil{
		maps["ret"] = 1
		maps["error"] = err.Error()
		return maps, nil
...
func (f *ApiPackageService) Put (ctx context.Context,r *rest.Request) (interface{}, error) {
	logger.Finest("ApiPackageService Put function");
	...
	cnt, err := models.PutApiPkgService(id, param)
	if err != nil{
		maps["ret"] = 1
		maps["error"] = err.Error()
		return maps, nil
...
func (f *ApiPackageService) Delete (ctx context.Context,r *rest.Request) (interface{}, error) {
	logger.Finest("ApiPackageService Delete function");
	var maps map[string] interface{}
	maps = make(map[string] interface{})
	maps["ret"] = 0
	...
	num, err := models.DeleteApiPkgService(param)
	if err != nil{
		maps["ret"] = 1
		maps["error"] = err.Error()
		return maps, nil
