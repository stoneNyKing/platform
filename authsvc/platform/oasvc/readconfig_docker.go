...
func GetSettings() {
	//vip设置路径
	viper.SetConfigName("oasvc")
	viper.AddConfigPath("/etc/oasvc")
	...
	once.Do(func() {
		if config.Config == nil {
			...
			if err != nil {
				...
				if cmd.Sdt == "etcd" {
					err = viper.AddRemoteProvider("etcd", "http://"+ss[0], "/config/oasvc.json")
				} else {
					err = viper.AddRemoteProvider("consul", ss[0], "/config/oasvc.json")
				}
				if err != nil {
			...
			if cmd.ServiceConf != "" {
				if string([]byte(cmd.ServiceConf)[0]) == "/" {
					config.Config.ServiceConf = "/etc" + cmd.ServiceConf
			...
			if httpaddr != "" {
				config.Config.ContainerHttp = utils.Hostname2IPv4(httpaddr)
			}
			if httpsaddr != "" {
				config.Config.ContainerHttps = utils.Hostname2IPv4(httpsaddr)
			}

			if crpcxaddr != "" {
				config.Config.ContainerRpcx = utils.Hostname2IPv4(crpcxaddr)
			}

			config.Config.RecordAddr = config.Config.ContainerHttp
		}
	})
