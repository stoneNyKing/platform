...
func init() {
	...
	if imconf.Config.DbDriver == "mysql" {
		...
		err = orm.RegisterDataBase("objects", "mysql", imconf.Config.ObjectsdbDsn)
	} else if imconf.Config.DbDriver == "pgsql" {
		err = orm.RegisterDataBase("default", "postgres", imconf.Config.Dsn)
		err = orm.RegisterDataBase("objects", "postgres", imconf.Config.ObjectsdbDsn)
	}
...
func run() {
	...
	//rpcx.NewClientPool(imconf.Config.Sdt,imconf.Config.Sda,imconf.Config.RpcxConfBasepath,"ConfJSONRpc",client.Failtry,client.RoundRobin)
	//rpcx.NewClientPool(imconf.Config.Sdt,imconf.Config.Sda,imconf.Config.RpcxOamBasepath,"OamJSONRpc",client.Failtry,client.RoundRobin)
	//rpcx.NewClientPool(imconf.Config.Sdt,imconf.Config.Sda,imconf.Config.RpcxOaBasepath,"OaJSONRpc",client.Failtry,client.RoundRobin)
