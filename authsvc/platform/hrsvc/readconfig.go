...
func GetSettings() {
	//vip设置路径
	viper.SetConfigName("hrsvc")
	viper.AddConfigPath("/etc/hrsvc")
	...
	once.Do(func() {
		if imconf.Config == nil {
			...
			if err != nil {
				...
				if cmd.Sdt == "etcd" {
					err = viper.AddRemoteProvider("etcd", "http://"+ss[0], "/config/hrsvc.json")
				}else{
					err = viper.AddRemoteProvider("consul", ss[0], "/config/hrsvc.json")
				}
				if err != nil {
			...
			if cmd.AppdashAddr != "" {
				imconf.Config.AppdashAddr = utils.Hostname2IPv4(cmd.AppdashAddr)
			}
