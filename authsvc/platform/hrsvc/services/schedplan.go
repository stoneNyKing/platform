...
type SchedPlanService struct {
	rest.RestApi
}

func (f *SchedPlanService) Get(ctx context.Context,r *rest.Request) (interface{}, error) {

	logger.Finest("SchedPlanService get function.")
	...
	switch action {
	case "list":
		datas, count, err = models.GetSchedPlanLists(ctx,siteid, appid, stypes, contents, order, sort, num, start)
	case "count":
		count, err = models.GetSchedPlanCount(ctx,siteid, appid, stypes, contents, order, sort)
	default:
		if id > 0 {
			datas, count, err = models.GetSchedPlan(ctx,id, siteid, appid)
		}
	}
...
func (f *SchedPlanService) Post(ctx context.Context,r *rest.Request) (interface{}, error) {

	logger.Finest("SchedPlanService POST function.")
	...
	id, err = models.PostSchedPlan(ctx,siteid, appid, token, param)
...
func (f *SchedPlanService) Put(ctx context.Context,r *rest.Request) (interface{}, error) {

	logger.Finest("SchedPlanService Put function.")
	...
	cnt, err := models.PutSchedPlan(ctx,siteid, appid, id, token, param)
...
func (f *SchedPlanService) Delete(ctx context.Context,r *rest.Request) (interface{}, error) {

	logger.Finest("SchedPlanService Delete function.")
	...
	cnt, err := models.DeleteSchedPlan(ctx,id, param)
