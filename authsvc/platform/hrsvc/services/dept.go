...
func (f *DeptService) Get(ctx context.Context,r *rest.Request) (interface{}, error) {

	logger.Finest("DeptService get function.")
	...
	if id == 0 && action != "list" && action != "tree" && action != "count" {
	...
	switch action {
	case "list":
		datas, count, err = models.GetDeptLists(ctx,siteid, appid, stypes, contents, order, sort, num, start)
	case "tree":
		datas, err = models.GetDeptTree(ctx,siteid, appid, stypes, contents, order, sort)
	case "count":
		count, err = models.GetDeptCount(ctx,siteid, appid, stypes, contents, order, sort)

	default:
		if id > 0 {
			datas, count, err = models.GetDept(ctx,id, siteid, appid)
		}
	}
...
func (f *DeptService) Post(ctx context.Context,r *rest.Request) (interface{}, error) {

	logger.Finest("DeptService POST function.")
...
func (f *DeptService) Put(ctx context.Context,r *rest.Request) (interface{}, error) {

	logger.Finest("DeptService Put function.")
...
func (f *DeptService) Delete(ctx context.Context,r *rest.Request) (interface{}, error) {

	logger.Finest("DeptService Delete function.")
	...
	cnt, err := models.DeleteDept(ctx,siteid, id, param)
