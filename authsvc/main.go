...
import (
	...
	"platform/authsvc/router"
	"platform/authsvc/rpc"
...
func init() {
	...
	// 初始化redis
	redis.Init(imconf.Config.RedisAddr, imconf.Config.RedisDb, imconf.Config.RedisPool)
...
func rpcService(msApp *grace.MicroService,param map[string]interface{}) error {
	host := utils.ConvertToString(param["host"])
	port := utils.ConvertToString(param["port"])
	sdt := utils.ConvertToString(param["sd_type"])
	sda := utils.ConvertToString(param["sd_address"])
	...
	sdtop := rpcx.RpcxSdTypeOption(sdt)
	sdop := rpcx.RpcxSdAddressOption(sda)

	rpc.RpcServer(imconf.Config.RpcxNetwork, sda, imconf.Config.ContainerRpcx, imconf.Config.RpcxBasepath,zkoption,sdtop,sdop)
	return nil
}